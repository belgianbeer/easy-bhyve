...
easy-bhyveは最小限の設定でbhyveを利用できるのを目標に作った、bhyve用VMのマネージコマンドです。bhyveのオプションや設定を十分に理解していなくても、少しの設定で新たなVMを用意して利用できます。

またeasy-bhyveでは開発や実験目的で利用することに主眼に、VMの起動や終了を手軽に行うことを前提にしており、通常のコマンドと同様の感覚で起動や停止等を行う実装となっています。

さらにZFSと組み合わせることで、VMのクローンやスナップショット、ロールバックが実現でき、開発やテスト用のサーバーを気軽に作成・廃棄できます。

## easy-bhyveの特徴

easy-bhyveの特徴を並べるとざっくり次の通りです。
...
ただしeasy-bhyveではVM内部で再起動を行うとVM自体が終了し、起動しないという欠点もあります。前述のようにeasy-bhyveではVMの起動や終了を手軽行うことを前提にしているため、終了したら(ユーザーが)改めて起動すればよいという思想に基づいています。
